...
public static class MoveHistoryHandler
{
    ...
    {
        if (
            game.MoveHistory.Count >= move.MoveNum
            || Math.Abs(game.MoveHistory.Count - move.MoveNum) >= 2
        )
        {
            throw new Exception(
                "Move History has been changed outside of the MoveHistoryManager! Please start the game again!"
            );
        }
